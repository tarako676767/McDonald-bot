...
class PayPay():
    def __init__(self,phone:str=None,password:str=None,device_uuid:str=None,client_uuid:str=str(uuid4()),access_token:str=None,proxy=None):
        ...
        self.version="5.49.0" #BeautifulSoup(iosstore.text,"html.parser").find(class_="l-column small-6 medium-12 whats-new__latest__version").text.split()[1]
        device_state = generate_device_state()
        self.headers = {
            "Accept": "*/*",
        ...
        if access_token:
            ...
            headers = {
                ...
                "User-Agent": f"Mozilla/5.0 (Linux; Android 10; SCV38 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/136.0.0.0 Mobile Safari/537.36 jp.pay2.app.android/{self.version}",
            ...
            authorize_response = self.session.get(f"https://www.paypay.ne.jp/portal/api/v2/oauth2/authorize",headers=headers,params=params,proxies=self.proxy)
            
            # AWS WAF Challengeが返された場合は解決する
            if authorize_response.status_code == 202 and 'x-amzn-waf-action' in authorize_response.headers:
                user_agent = f"Mozilla/5.0 (Linux; Android 10; SCV38 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/136.0.0.0 Mobile Safari/537.36 jp.pay2.app.android/{self.version}"
                aws_waf_token = solve_aws_waf_challenge(authorize_response.text, user_agent)
            ...
            signin_page_response = self.session.get("https://www.paypay.ne.jp/portal/oauth2/sign-in",headers=headers,params=params,proxies=self.proxy)
            
            # AWS WAF Challengeが返された場合は解決する
            if signin_page_response.status_code == 202 and 'x-amzn-waf-action' in signin_page_response.headers:
                user_agent = f"Mozilla/5.0 (Linux; Android 10; SCV38 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/136.0.0.0 Mobile Safari/537.36 jp.pay2.app.android/{self.version}"
                aws_waf_token = solve_aws_waf_challenge(signin_page_response.text, user_agent)
                self.session.cookies.set("aws-waf-token", aws_waf_token, domain=".paypay.ne.jp")
                # 再度リクエストを送信
